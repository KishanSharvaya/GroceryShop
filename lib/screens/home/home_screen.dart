...
class _HomeScreenState extends BaseState<HomeScreen>
  ...
  String Address = "Test Address";/*SharedPrefHelper.instance.getSignUpData().Address == null
      ? ""
...
//inside initState method
  ...
  void initState() {
    ...
    _categoryScreenBloc..add(CategoryListRequestCallEvent(CategoryListRequest(BrandID: "",ProductGroupID: "",ProductID: "",CompanyId: CompanyID)));
    _categoryScreenBloc..add(ExclusiveOfferListRequestCallEvent(ExclusiveOfferListRequest(PercentTo: "01",PercentFrom: "30",CompanyId: CompanyID)));
  ...
  void _onCategoryResponse(CategoryListResponseState state, BuildContext context) {
   ...
   String imagePath;*/
      if (state.response.details[i].productGroupID == 1)
        ...
        groceryItem.ProductImage = "http://122.169.111.101:206/"+state.response.details[i].productImage;;
...
*/
  ...
  void _onExclusiveOfferListResponse(ExclusiveOfferListResponseState state, BuildContext context) {
...
*/
  ...
  void _onCartListResponse(ProductCartResponseState state, BuildContext context)  async {
   // var value = await OfflineDbHelper.getInstance().getProductCartList();
    ...
    List<ProductCartModel> Tempgetproductlistfromdbtest = await OfflineDbHelper.getInstance().getProductCartList();

    setState(() {
      _count = Tempgetproductlistfromdbtest.length;
...
@override
  ...
  void _onFavoriteListResponse(ProductFavoriteResponseState state, BuildContext context) async {



    List<ProductCartModel> Tempgetproductlistfromdb = await OfflineDbHelper.getInstance().getProductCartFavoritList();
