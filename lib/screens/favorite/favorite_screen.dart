...
class _FavoriteItemsScreenState extends BaseState<FavoriteItemsScreen> with BasicScreen,WidgetsBindingObserver {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          AppText(
                            text: "\$${groceryItem.UnitPrice.toStringAsFixed(2)}",
                            fontSize: 12,
                            fontWeight: FontWeight.bold,
                          ),
                          Spacer(),
                          ...
                          InkWell(
                              onTap: (){
                                ...
                                _onTapOfDeleteContact(context,groceryItem.ProductID);
                              },
                              child: DeleteWidget())
                        ],
                      )
 ...
 getproductductfavoritedetails() async {
   ...
   /* if(groceryItemdb.isNotEmpty)
    ...
    for(int i=0;i<groceryItemdb.length;i++)
    {
      ...
      groceryItem.ProductImage = groceryItemdb[i].ProductImage;//==""?"https://img.icons8.com/bubbles/344/no-image.png":"http://122.169.111.101:206/"+state.response.details[i].productImage;
    ...
    if(groceryItemdb.isNotEmpty)
      {
        ...
        _categoryScreenBloc.add(FavoriteDeleteRequestCallEvent(CustomerID,CartDeleteRequest(CompanyID: CompanyID)));
        for(int i=0;i<groceryItemdb.length;i++)
        {
          ...
          cartModel.DiscountPercent=groceryItemdb[i].DiscountPercent==null?0.00:groceryItemdb[i].DiscountPercent;
        ...
        _categoryScreenBloc.add(InquiryFavoriteProductSaveCallEvent(arrCartAPIList));
  ...
  void _onCategoryResponse(InquiryFavoriteProductSaveResponseState state, BuildContext context) {
    print("FavoriteSaveResponse"+ " Details " + state.inquiryProductSaveResponse.details[0].column2);

  }

  void _OnCartDeleteRequest(FavoriteDeleteResponseState state, BuildContext context) {

    print("FavoriteSaveResponse"+ " Details " + state.cartDeleteResponse.details[0].column1.toString());
